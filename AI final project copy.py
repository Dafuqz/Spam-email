...
list_of_words = many_dict20spam(spam1)

list_of_hwords = many_dict20ham(ham1)
...
for i in range(len(list_of_hprobs)):
    if list_of_hprobs[i] > list_of_sprobs[i]:
        ...
        choice.append(random.randint(0,1))
print(choice)
mark1 = 0
for item in choice:
    if item == 1 and choice.index(item) <= 19:
        mark1 += 1
        print (" ham -----> correct")
    elif item == 0 and choice.index(item)> 19:
        mark1 += 1
        print ("spam ----->  correct")
    else:
        if item == 0:
...
mark1 = mark1/float(len(choice))



print( "The accuracy is: ",  "--> is about---->", round (mark1*100, 1), "%")